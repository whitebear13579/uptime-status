    ...
    <div key={site.id} className='site'>
      ...
      <div className='timeline'>
        {site.daily.map((data, index) => {
          ...
          if (data.uptime >= 100) {
            status = 'ok';
            text += `可使用率 ${formatNumber(data.uptime)}%`;
          }
            ...
            status = 'down';
            text += `故障 ${data.down.times} 次，累計 ${formatDuration(data.down.duration)}，可使用率 ${formatNumber(data.uptime)}%`;
          }
      ...
      <div className='summary'>
        ...
        <span>
          {site.total.times
            ? `最近 ${CountDays} 天故障 ${site.total.times} 次，累計 ${formatDuration(site.total.duration)}，平均可使用率 ${site.average}%`
            : `最近 ${CountDays} 天可使用率 ${site.average}%`}
        </span>
